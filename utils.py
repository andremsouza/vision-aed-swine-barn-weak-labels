...
def train(
    ...
    for epoch in range(num_epochs):
        ...
        epoch_acc: float = float(running_corrects) / float(len(train_loader.dataset) * n_classes)  # type: ignore # noqa: E501 # pylint: disable=line-too-long
        # TODO: Fix epoch_f1
        ...
        if verbose:
            ...
            print(
                f"Training Loss: {epoch_loss:.4f} Acc: {epoch_acc:.4f} F1: {epoch_f1:.4f}"
            )
...
def test(
    ...
    epoch_acc: float = float(running_corrects) / float(len(test_loader.dataset))  # type: ignore
    epoch_f1: float = sklearn.metrics.f1_score(
