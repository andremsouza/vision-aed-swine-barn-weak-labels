...
N_LAYERS: list[int] = args.n_layers
M_UNITS: list[int] = args.m_units
...
if __name__ == "__main__":
    ...
    dataset_train_spec.transform = lambda x: torch.flatten(
        ((x - train_mean_spec) / train_std_spec)[None, :, :].transpose(1, 2)
    )
    dataset_test_spec.transform = lambda x: torch.flatten(
        ((x - train_mean_spec) / train_std_spec)[None, :, :].transpose(1, 2)
    )
    ...
    for n_layers in N_LAYERS:
        for m_units in M_UNITS:
            for train_dataloader, test_dataloader in zip(
                ...
                experiment_name: str = (
                    f"{EXPERIMENT_PREFIX}_{n_layers}-layers_{m_units}-units_"
